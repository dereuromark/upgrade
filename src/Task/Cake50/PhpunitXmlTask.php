...
class PhpunitXmlTask extends Task implements RepoTaskInterface
{
  ...
  */
    ...
    public function run(string $path): void
    {
        ...
        $newContent = $this->replaceFilterWithCoverage($newContent);
    ...
    protected function addFixtureSchemadataPath(string $path, string $content): string
    {
        ...
        if (empty($this->config['skipSchemaCheck']) && !$this->containsFixtures($files)) {
            return $content;
        }
...
TXT;
     ...
     * @return string
     */
    protected function replaceFilterWithCoverage(string $content): string
    {
        $content = str_replace('<filter>', '<coverage>', $content);
        $content = str_replace('</filter>', '</coverage>', $content);
        $content = str_replace('<whitelist>', '<include>', $content);
        $content = str_replace('</whitelist>', '</include>', $content);
        $content = str_replace('<blacklist>', '<exclude>', $content);
        $content = str_replace('</blacklist>', '</exclude>', $content);
